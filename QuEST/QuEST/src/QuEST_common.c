...
#define macro_setConjugateMatrix(dest, src, dim)                            \
    ...
    long int temp = (tv.tv_sec) * 1000 + (tv.tv_usec) / 1000; // convert tv_sec & tv_usec to millisecond
    unsigned long int pid = getpid();
    unsigned long int msecs = (unsigned long int)temp; //time_in_mill_2;
...
#define macro_allocStackComplexMatrixN(matrix, numQubits)                            \
    ...
    void densmatr_mixPauli(Qureg qureg, int qubit, qreal probX, qreal probY, qreal probZ)
    {
        ...
        qreal facs[4] = {// literal numOps=4 for valid initialisation
                         ...
                         sqrt(probY),
                         sqrt(probZ)};
        ops[0].real[0][0] = facs[0];
        ops[0].real[1][1] = facs[0];
        ops[1].real[0][1] = facs[1];
        ops[1].real[1][0] = facs[1];
        ops[2].imag[0][1] = -facs[2];
        ops[2].imag[1][0] = facs[2];
        ops[3].real[0][0] = facs[3];
        ops[3].real[1][1] = -facs[3];
