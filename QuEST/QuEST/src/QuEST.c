 ...
 */
    ...
    void createQureg_debug(Qureg *qureg, int numQubits, QuESTEnv env)
    {
        validateNumQubitsInQureg(numQubits, env.numRanks, __func__);
 ...
 */
    ...
    void compactUnitary(Qureg qureg, const int targetQubit, Complex alpha, Complex beta, QuESTEnv env)
    {
        // validateTarget(qureg, targetQubit, __func__);
        // validateUnitaryComplexPair(alpha, beta, __func__);

        statevec_compactUnitary(qureg, targetQubit, alpha, beta, env);

        // if (qureg.isDensityMatrix)
        // {
        //     int shift = qureg.numQubitsRepresented;
        //     statevec_compactUnitary(qureg, targetQubit + shift, getConjugateScalar(alpha), getConjugateScalar(beta), env);
        // }
