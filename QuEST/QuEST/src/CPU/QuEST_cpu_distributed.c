...
{
    long long int sizeOuterHalfBlock = 1LL << (targetQubit + numQubits);
    if (chunkSize > sizeOuterHalfBlock)
        return 1;
    else
        return 0;
}
...
// general for last two cases
void exchangeStateVectors_adaptive(Qureg qureg, int pairRank, QuESTEnv env, const int targetQubit)
{
    ...
    long long int msgExchange = qureg.numAmpsPerChunk;
    long long int offset = 0;
    long long int offset_pair = 0;
    if (env.rank == 1)
    {
        msgExchange = qureg.numAmpsPerChunk / 3;
        if (targetQubit == qureg.numQubitsInStateVec - 2)
        {
            offset = 0 * msgExchange;
        }
        else if (targetQubit == qureg.numQubitsInStateVec - 1)
        {
            offset = 1 * msgExchange;
        }
    }

    // send my state vector to pairRank's qureg.pairStateVec
    // receive pairRank's state vector into qureg.pairStateVec
    MPI_Sendrecv(&qureg.stateVec.real[offset], msgExchange, MPI_QuEST_REAL, pairRank, TAG,
                 &qureg.pairStateVec.real[offset_pair], msgExchange, MPI_QuEST_REAL,
                 pairRank, TAG, MPI_COMM_WORLD, &status);
    //printf("rank: %d err: %d\n", qureg.rank, err);
    MPI_Sendrecv(&qureg.stateVec.imag[offset], msgExchange, MPI_QuEST_REAL, pairRank, TAG,
                 &qureg.pairStateVec.imag[offset_pair], msgExchange, MPI_QuEST_REAL,
                 pairRank, TAG, MPI_COMM_WORLD, &status);
}
...
#pragma omp parallel default(none)                                                                                                                                                \
    shared(sizeInnerBlock, sizeInnerHalfBlock, sizeOuterColumn, sizeOuterHalfColumn, qureg) private(thisTask, thisInnerBlock, thisOuterColumn, thisIndex, thisIndexInOuterColumn, \
...
#pragma omp parallel default(none)                                                                                                \
    shared(sizeInnerBlockQ1, sizeInnerHalfBlockQ1, sizeInnerQuarterBlockQ2, sizeInnerHalfBlockQ2, sizeInnerBlockQ2,               \
           sizeOuterColumn,                                                                                                       \
           sizeOuterQuarterColumn, qureg) private(thisTask, thisInnerBlockQ2, thisOuterColumn, thisIndex, thisIndexInOuterColumn, \
...
void statevec_compactUnitary(Qureg qureg, const int targetQubit, Complex alpha, Complex beta, QuESTEnv env)
{
    ...
    if (targetQubit < qureg.numQubitsInStateVec - 2)
    {
        printf("rank: %d, transmission time: %Lf\n", env.rank, 0);
        ...
        printf("rank: %d, calculation time: %Lf\n", env.rank, endTime - startTime);
    }
    else
    {
        printf("rank: %d, last but one\n", env.rank);
        /* ----------- Start ------------- */
        if (env.rank == 0)
        {
            rankIsUpper = 1; // the same as pairRank
            updateTasks = qureg.numAmpsPerChunk;
        }
        else
        {
            rankIsUpper = 0;
            updateTasks = qureg.numAmpsPerChunk / 3;
        }
        getRotAngle(rankIsUpper, &rot1, &rot2, alpha, beta);

        // start timing execution, Teng check
        struct timeval timeInstance;
        gettimeofday(&timeInstance, NULL);
        long double startTime = (timeInstance.tv_sec + (long double)timeInstance.tv_usec / pow(10, 6));
        ...
        exchangeStateVectors_adaptive(qureg, rankIsUpper, env, targetQubit); // get the lower from rank 1 // last 2 cases are considered
        ...
        if (env.rank == 0)
        {
            nonLocalShift = 0; // always
            rankIsUpper = 1;
            getRotAngle(rankIsUpper, &rot1, &rot2, alpha, beta);
            statevec_compactUnitaryDistributed_adaptive(qureg, rot1, rot2,
        ...
        else
        {
            ...
            int isUpper;
            Complex rot1_up, rot2_up;
            isUpper = 1;
            getRotAngle(isUpper, &rot1_up, &rot2_up, alpha, beta);
            Complex rot1_lo, rot2_lo;
            isUpper = 0;
            getRotAngle(isUpper, &rot1_lo, &rot2_lo, alpha, beta);

            statevec_compactUnitaryDistributed_adaptive_local(qureg, rot1_up, rot2_up, rot1_lo, rot2_lo,
                                                              qureg.stateVec,
                                                              updateTasks,
                                                              localShift_up,
                                                              localShift_lo);
            ...
            // isUpper = 1;
            // getRotAngle(isUpper, &rot1, &rot2, alpha, beta);
            // statevec_compactUnitaryDistributed_adaptive_local(qureg, rot1, rot2,
            //                                                   qureg.stateVec,
            //                                                   updateTasks,
            //                                                   localShift_up,
            //                                                   localShift_lo,
        ...
        // stop timing execution
        gettimeofday(&timeInstance, NULL);
        endTime = (timeInstance.tv_sec + (long double)timeInstance.tv_usec / pow(10, 6));
        printf("rank: %d, calculation time: %Lf\n", env.rank, endTime - startTime);
    }
}
...
void statevec_collapseToKnownProbOutcome(Qureg qureg, const int measureQubit, int outcome, qreal totalStateProb)
{
    ...
    else
    {
        if (!isChunkToSkipInFindPZero(qureg.chunkId, qureg.numAmpsPerChunk, measureQubit))
        {
            ...
            if (outcome == 0)
                statevec_collapseToKnownProbOutcomeDistributedRenorm(qureg, measureQubit,
                                                                     totalStateProb);
            else
                statevec_collapseToOutcomeDistributedSetZero(qureg);
        }
        else
        {
