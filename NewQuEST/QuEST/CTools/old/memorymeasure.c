...
int getMemory(
	...
	while (fscanf(file, " %1023s", buffer) == 1) {
		...
		if (strcmp(buffer, "VmRSS:") == 0) {	// Resident set size (in RAM). Note that the value here is the sum of RssAnon, RssFile, and RssShmem.
			fscanf(file, " %lu", currRealMem);
		}
		if (strcmp(buffer, "VmHWM:") == 0) {	// Peak resident set size ("high water mark"). 
			fscanf(file, " %lu", peakRealMem);
		}
		if (strcmp(buffer, "VmSize:") == 0) {	// Virtual Memory Size
			fscanf(file, " %lu", currVirtMem);
		}
		if (strcmp(buffer, "VmPeak:") == 0) {	// Peak Virtual Memory Size
